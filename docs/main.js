    ...
    add: (id, qty) => {
      ...
      if (cartItem) {
        if (shoppingCart.methods.hasInventory(id, qty + cartItem.qty)) {
          ...
          alert('There is not inventory for this product');
        }
      } else {
  ...
  document.querySelectorAll('.item .actions .add').forEach((button) => {
    button.addEventListener('click', (e) => {
      ...
      if (item && item.qty - 1 > 0) {
        ...
        console.log('There is not inventory for this product');
      }
    });
  });
}
...
function renderShoppingCart() {
  ...
  <button class="button button-purchase">Purchase</button>
  </div>`
      : '';
  ...
  document
    .querySelector('.button .button-close')
    .addEventListener('click', (event) => {});
  docuement
    .querySelector('.button .button-purchase')
    .addEventListener('click', (event) => {});
}
